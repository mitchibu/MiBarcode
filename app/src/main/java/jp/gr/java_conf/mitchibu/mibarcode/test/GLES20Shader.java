package jp.gr.java_conf.mitchibu.mibarcode.test;
...
public class GLES20Shader {
	...
	public void setup() {
		release();
		mVertexShader   = GLEngine.loadShader(GLES20.GL_VERTEX_SHADER,   mVertexShaderSource);
		mFragmentShader = GLEngine.loadShader(GLES20.GL_FRAGMENT_SHADER, mFragmentShaderSource);
...
//		mVertexBufferName   = GLES20Utils.createBuffer(VERTICES_DATA);
		FloatBuffer fb3 = BufferUtils.wrapFloatBuffer(VERTICES_DATA);
		int[] buffer = new int[1];
		GLES20.glGenBuffers(buffer.length, buffer, 0);
		mVertexBufferName = buffer[0];
		GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, buffer[0]);
		GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER, fb3.capacity() * BufferUtils.BYTES_FLOAT, fb3, GLES20.GL_STATIC_DRAW);
	}
